...
def GetAssemblyDistrictsInBounds(assembly_districts_file:IO, bounds_gdf = None):	
	...
	if assembly_districts_file.exists():
		...
		else: gdf = gpd.read_file(assembly_districts_file)
		_, gdf = ComputeRegionCentroids(gdf)
		gdf = gdf.rename(columns={"ASM2021": 'id'})
		gdf['z_layer'] = [0]*len(gdf)
		gdf.reset_index(inplace=True)
...
def GetSenateDistrictsInBounds(senate_districts_file:IO, bounds_gdf = None):	
	...
	if senate_districts_file.exists():
		...
		else: gdf = gpd.read_file(senate_districts_file)
		_, gdf = ComputeRegionCentroids(gdf)		
		gdf = gdf.rename(columns={"SEN2021": 'id'})
		gdf['z_layer'] = [0]*len(gdf)
		gdf.reset_index(inplace=True)
...
def GetPublicSchoolDistrictsInBounds(school_districts_file:IO, county_bounds, county_name:str, city_name:str):
	"""
	...
	if school_districts_file.exists(): 
		...
		if not gdf.empty:
			return _extracted_from_GetPublicSchoolDistrictsInBounds_23(gdf, county_bounds)
...
def GetCityInCounty(county_ward_gdf, county_name, city_name):
	...
	"""
	gdf = county_ward_gdf.loc[(county_ward_gdf['CNTY_NAME'] == county_name) & (county_ward_gdf['MCD_NAME'] == city_name)].copy(deep = True)
...
def GetWardDataFromList(in_file:IO, county_list, headers, numeric, geometry):
	...
	if in_file.exists():
		...
		gdf = gdf.rename(columns = {'PERSONS':'Total', 'PERSONS18':'VAP', 
			'HISPANIC':'LATINX', 'HISPANIC18':'LATINX18'})

		gdf = gdf.rename(columns = {'WHITE18':'WhiteVAP','BLACK18':'BlackVAP',
